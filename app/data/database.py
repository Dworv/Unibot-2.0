...
class Database:
    def __init__(self):
        self.con = sqlite3.connect(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "unibot.db")
        )
    ...
    # application methods
    def new_application(
        self, applicant_id: int, review_msg_id: int, status: str, url: str
    ) -> Application:
        data = self.editor.create_application(
            applicant_id, review_msg_id, status, url, int(time.time())
        )

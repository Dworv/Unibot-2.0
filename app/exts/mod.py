...
class Mod(EnhancedExtension):
    ...
    async def mute(
        ...
        reason: EnhancedOption(
            str, "The reason you are muting them (optional)"
        ) = "Not provided",
    ):
        ...
        await target.modify(
            ctx.guild_id, communication_disabled_until=muted_until.isoformat()
        )
        await ctx.send(
            embeds=build_embed(
                f"{target.mention} has been muted until <t:{int(mktime(muted_until.timetuple()))}:F>"
            )
        )

...
class Apply(EnhancedExtension):
    ...
    async def apply(
        ...
        applications: Application = self.client.database.search_applications(
            "applicant_id", int(ctx.author.id)
        )
        ...
        if not validators.url(url):
            await ctx.send(
                embeds=build_embed(
                    "The link you used appeaars to be invalid. Please use a valid url."
                )
            )
            return
        ...
        reviewing_channel = self.client.get(
            inter.Channel, channel_id=const.METADATA["channel"]["reviewing"]
        )
        ...
        self.client.database.new_application(
            int(ctx.author.id), int(reviewing_msg.id), 0, url
        )
